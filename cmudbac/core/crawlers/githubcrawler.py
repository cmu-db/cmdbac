...
class GitHubCrawler(BaseCrawler):
    ...
    def add_repository(self, name, setup_scripts = None):
        if Repository.objects.filter(name=name, source=self.crawlerStatus.source).exists():
            LOG.info("Repository %r already exists", name)
            return
        ...
        LOG.info("Found new repository %r", name)
        ...
        repo.created_at = pytz.utc.localize(datetime.strptime(api_data['created_at'], "%Y-%m-%dT%H:%M:%SZ"))
        repo.updated_at = pytz.utc.localize(datetime.strptime(api_data['updated_at'], "%Y-%m-%dT%H:%M:%SZ"))
        repo.pushed_at = pytz.utc.localize(datetime.strptime(api_data['pushed_at'], "%Y-%m-%dT%H:%M:%SZ"))
    ...
    def get_api_data(self, name):
        response = self.github_query(urllib.parse.urljoin(API_GITHUB_REPO, name))
        data = response.json()
        return data
    ## DEF
    ...
    def get_webpage_data(self, name):
        data = {}
        response = self.github_query(urllib.parse.urljoin(GITHUB_HOST, name))
    ...
    def next_url(self):
        ...
        args = {
            'query': urllib.parse.urlencode({ 'q': self.crawlerStatus.query})[2:] if self.crawlerStatus.query != '' else '',
    ...
    def search(self):
        ...
        repositories = [item['repository'] for item in response_json['items']]
