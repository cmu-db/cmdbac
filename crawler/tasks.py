...
def runserver(directory, managePath):
    threshold = 10
    line = 'placeholder'
    #for time in range(threshold):
    command = 'vagrant ssh -c ' + "'python /vagrant/" + directory + managePath + " runserver > /vagrant/log 2>&1 & sleep 10'"
    run_command(command)
    logFile = open('log', 'r')
...
def installRequirements(directory, requirementsPath, app_type):
    if app_type.app_type == 'Django: Application':
        ...
        command = run_command("vagrant ssh -c 'sudo pip install -r /vagrant/" + directory + requirementsPath + "'")
    ...
    while True:
        ...
        for app in apps:
            ...
            if app.models_path != '':
                ...
                app.model_size = 0
            if app.manage_path != '':
                result = runserver(directoryName, app.manage_path)
            else:
